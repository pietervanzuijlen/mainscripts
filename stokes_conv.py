    ...
    for mid in M1: 
        ...
        for error in ['residual','sum_residual','sum_goal']:
            ...
            for i, method in enumerate(methods):
                ...
                text = writer.read('../results/stokes/'+case+methods[i]+str(mid))
            ...
            slopemarker = {}
            if error == 'sum_goal':
                slopemarker['uniform'] = [(-1,1),.06]
            elif error == 'residual':
                slopemarker['uniform'] = [(-1,2),.12]
                if mid == 0.35:
                    slopemarker['uniform'] = [(-3,5),1.3]
            elif error == 'sum_residual':
                slopemarker['uniform'] = [(-5,3),.18]

            labels = ['amount of elements','error']

            with treelog.add(datalog):
                plotter.plot_convergence(str(mid)+'-'+error, xval, yval, slopemarker=slopemarker, levels=level)
