    ...
    for mid in [0.5,0.4,0.35]:
        ...
        for method in methods:
            ...
            for nref in range(maxref):
                ...
                ns.evalbasis = domain.basis('th-spline', degree=degree)
...
#    ns.inflow = '( g_i + stress_ij n_j ) ( z_i - Iz_i )'
#    ns.outflow = '( stress_ij n_j ) ( z_i - Iz_i )'
#    ns.jump  = '( [-stress_ij] n_j ) ( z_i - Iz_i )'
#    ns.force = '( stress_ij,j ) (z_i - Iz_i)'
#    ns.incom = 'u_i,i (s - Is)'

    #ns.bound = '( g_i - stress_in n_n ) ( z_i - Iz_i )'
    #ns.jump  = '( [-stress_ij] n_j ) ( z_i - Iz_i )'
    #ns.force = '( stress_ij,j ) ( z_i - Iz_i )'
    #ns.incom = 'u_i,i (s - Is)'

#    inflow = function.abs(ns.inflow)*function.J(geom)
#    outflow = function.abs(ns.outflow)*function.J(geom)
#    jump  = function.abs(ns.jump)*function.J(geom)
#    force = function.abs(ns.force)*function.J(geom)
#    incom = function.abs(ns.incom)*function.J(geom)
#
#    goal_indicators  = indicater.elementbased(domain, geom, degree)
#    int_indicators   = indicater.elementbased(domain, geom, degree)
...
#    bound_indicators = indicater.elementbased(domain, geom, degree)
#
#    goal_indicators  = goal_indicators.goaloriented(dualspace, force, 'internal')
#    goal_indicators  = goal_indicators.goaloriented(dualspace, incom, 'internal')
#    int_indicators   = int_indicators.goaloriented(dualspace, force, 'internal')
#    int_indicators   = int_indicators.goaloriented(dualspace, incom, 'internal')
#
#    goal_indicators  = goal_indicators.goaloriented(dualspace.interfaces, jump, 'interface')
#    jump_indicators  = jump_indicators.goaloriented(dualspace.interfaces, jump, 'interface')
#
#    goal_indicators  = goal_indicators.goaloriented(dualspace.boundary['left'], inflow, 'boundary')
#    bound_indicators = bound_indicators.goaloriented(dualspace.boundary['left'], inflow, 'boundary')
#
#    goal_indicators  = goal_indicators.goaloriented(dualspace.boundary['right'], outflow, 'boundary')
#    bound_indicators = bound_indicators.goaloriented(dualspace.boundary['right'], outflow, 'boundary')

    #ns.interior1 = '2 mu (u_i,j (z_i,j - Iz_i,j) + u_j,i (z_i,j - z_i,j))'
    #ns.interior2 = 'p (z_i,i - Iz_i,i)'
    #ns.interior3 = '( s - Is ) u_i,i'
    #ns.neumann  = 'g_i (z_i - Iz_i)'

    #gint    = function.abs(ns.gint)*function.J(geom)
    #gjump   = function.abs(ns.gjump)*function.J(geom)
    #gbound1 = function.abs(ns.gbound1)*function.J(geom)
    #gbound2 = function.abs(ns.gbound2)*function.J(geom)
    #gbound3 = function.abs(ns.gbound3)*function.J(geom)
    #gbound4 = function.abs(ns.gbound4)*function.J(geom)
    ...
    #goal_indicators = goal_indicators.goaloriented(dualspace.interfaces, gjump, 'interface')
    #jump_indicators = jump_indicators.goaloriented(dualspace.interfaces, gjump, 'interface')
