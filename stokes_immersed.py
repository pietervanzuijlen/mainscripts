      ...
      for method in methods:
        ...
        for nref in range(refinements):
          ...
          if method == 'goaloriented':
              ...
              # assemble indicaters
              indicators = incom*s_int + force*z_int + outflow*z_outflow

              plotter.plot_indicators('contributions_'+method+'_'+str(nref), domain, geom, {'momentum':force,'dualvelocity':z_int,'incompressibility':incom,'dualpressure':s_int,'dualoutflow':z_outflow}, normalize=False, alpha=.5)
          ...
          if method == 'residualbased':
              ...
              # assemble indicaters
              indicators = (incom + force)*h + outflow*np.sqrt(h)

              plotter.plot_indicators('indicators_'+method+'_'+str(nref), domain, geom, {'indicator':indicators,'incompressibility':incom*h,'momentum':force*h,'outflow':outflow*np.sqrt(h)}, normalize=False, alpha=.5)
